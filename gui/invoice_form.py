...
class InvoiceForm(tk.Frame):
    def __init__(self, parent):
        ...
        # Description
        tk.Label(self, text=_("Description :")).grid(row=1, column=0, sticky="e")
        self.description_entry = tk.Entry(self, width=40)
        self.description_entry.grid(row=1, column=1, columnspan=2, pady=5)

        # Quantity (hours)
        tk.Label(self, text=_("Quantity (h) :")).grid(row=2, column=0, sticky="e")
        self.qty_entry = tk.Entry(self)
        self.qty_entry.grid(row=2, column=1, pady=5)

        # Price per hour
        tk.Label(self, text=_("Price / hours (€) :")).grid(row=3, column=0, sticky="e")
        ...
        # List of items
        self.tree = ttk.Treeview(self, columns=(_("Description"), _("Quantity"), _("Price"), _("Total")), show="headings")
        for col in ("Description", "Quantity", "Price", "Total"):
